...
export async function GET() {
  ...
  const initialized = db
    .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='reference_candidates'")
    .get();
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (body.action === 'sync-reference')
      return NextResponse.json({ status: 'success', ...initializeReference(auth.user.id) });
