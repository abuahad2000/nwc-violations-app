...
import { violationManagerKeySQL, violationManagerNameSQL, programKeySQL, programNameSQL } from '@/lib/domain/manager';
import {
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const rows = (await db
      .prepare(
        `SELECT COALESCE(p.executive_director_name,'') executive,COALESCE(p.subprogram_name,'') subprogram,${programKeySQL} program_key,min(${programNameSQL}) program_name,${violationManagerKeySQL} manager_key,min(${violationManagerNameSQL}) manager_name,count(*) total,COALESCE(sum(CASE WHEN v.is_closed=0 THEN 1 ELSE 0 END),0) pending,COALESCE(sum(CASE WHEN v.is_closed=0 AND v.source_status='تحت معالجة المقاول' THEN 1 ELSE 0 END),0) contractor FROM current_violations v LEFT JOIN projects p ON p.id=v.project_id LEFT JOIN contractors c_proj ON c_proj.id=v.project_contractor_id WHERE ${whereSQL} GROUP BY COALESCE(p.executive_director_name,''),COALESCE(p.subprogram_name,''),${programKeySQL},${violationManagerKeySQL}`,
      )
