  ...
  return (
    <section aria-label="الإحصائيات الرئيسية" className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {cards.map(({ key, label, icon: Icon, tone }) => (
        <button key={key} onClick={() => onSelect?.(key)} className="group rounded-2xl border border-slate-200 bg-white p-5 text-start shadow-sm transition hover:-translate-y-0.5 hover:shadow-md" aria-label={`عرض ${label}`}>
          <div className="flex items-start justify-between gap-3">
            <span className={`flex h-11 w-11 items-center justify-center rounded-2xl ring-1 ${tones[tone]}`}><Icon size={22} aria-hidden="true" /></span>
            <span className="text-sm text-slate-500">{label}</span>
          </div>
          {loading ? <div className="mt-5 h-9 w-24 animate-pulse rounded-lg bg-slate-200" aria-label="جارٍ التحميل" /> : <strong className="mt-5 block text-3xl tracking-tight text-[#173f43]">{values[key].toLocaleString('ar-SA')}</strong>}
