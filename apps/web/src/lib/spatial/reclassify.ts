...
export async function reclassify(userId: string) {
  const boundaries = await db
    ...
    .all();
  const active = (await db.prepare("SELECT count(*) n FROM projects WHERE status='ACTIVE'").get())!;
  const matched = new Set(boundaries.map((b) => b.project_id));
  const complete = Number(active.n) > 0 && matched.size === Number(active.n);
  const snapshot = JSON.stringify(
    await db.prepare('SELECT id,updated_at FROM violations ORDER BY id').all(),
  );
  const referenceSnapshot = JSON.stringify(
    await db
      .prepare(
        'SELECT b.id,b.is_approved,b.project_id,p.status FROM project_boundaries b JOIN projects p ON p.id=b.project_id ORDER BY b.id',
      )
      .all(),
  );
  const points = (await db
    .prepare('SELECT id,latitude,longitude FROM violations')
    .all()) as SpatialInput[];
  ...
  return await db.transaction(async () => {
    ...
    for (const result of results) {
      const group =
        result.classification === 'INSIDE_ACTIVE_PROJECT'
          ...
          : result.classification === 'OUTSIDE_ACTIVE_PROJECTS'
            ...
            : 'UNDER_REVIEW';
      const old = (await db.prepare('SELECT * FROM violations WHERE id=?').get(result.id))!;
      await db.prepare('INSERT INTO audit_events VALUES (?,?,?,?,?,?,?)').run(
        crypto.randomUUID(),
      ...
      if (owner && !old.current_action_owner_id && !old.is_closed) {
        ...
        await db
          .prepare('UPDATE violations SET current_action_owner_id=? WHERE id=?')
          .run(owner, result.id);
      }
    }
