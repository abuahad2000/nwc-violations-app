...
export function initializeReference(userId: string) {
  const data = JSON.parse(
    fs.readFileSync(path.join(process.cwd(), 'data/reference-review.json'), 'utf8'),
  ) as { projects: Project[]; boundaries: Boundary[] };
  db.exec(`CREATE TABLE IF NOT EXISTS reference_candidates (id TEXT PRIMARY KEY, name TEXT NOT NULL, source_file TEXT NOT NULL, color TEXT, geometry_json TEXT NOT NULL, proposed_project_id TEXT, match_method TEXT, candidates_json TEXT, approved INTEGER NOT NULL DEFAULT 0);
 ...
 CREATE TABLE IF NOT EXISTS project_boundaries_before_reference AS SELECT * FROM project_boundaries;`);
  ...
  try {
    ...
    db.exec("UPDATE projects SET status='REVIEW'");
    for (const project of data.projects) {
      ...
      if (!contractor) {
        db.prepare('INSERT INTO contractors VALUES (?,?,1,?)').run(
    ...
    if(currentIds.length)db.prepare(`UPDATE project_boundaries SET is_approved=0 WHERE id IN (SELECT id FROM reference_candidates) AND id NOT IN (${currentIds.map(()=>'?').join(',')})`).run(...currentIds);
...
export async function approveBoundary(boundaryId: string, projectId: string, userId: string) {
  const candidate = db.prepare('SELECT * FROM reference_candidates WHERE id=?').get(boundaryId);
  const project = db.prepare('SELECT * FROM projects WHERE id=?').get(projectId);
  if (!candidate || !project || project.status === 'REVIEW')
    throw new Error('اختر نطاقًا ومشروعًا من المرجع المعتمد');
  if(candidate.approved){if(candidate.proposed_project_id===projectId)return;throw new Error('الربط معتمد؛ تعديل المشروع يتطلب إصدار نطاق جديد');}
  ...
  try {
    ...
    db.prepare('UPDATE reference_candidates SET approved=1,proposed_project_id=? WHERE id=?').run(
      projectId,
      boundaryId,
    );
    db.prepare('INSERT INTO audit_events VALUES (?,?,?,?,?,?,?)').run(
